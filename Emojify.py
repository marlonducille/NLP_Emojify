...
index = 4
print(X_train, label_to_emoji(Y_train[index]))
...
emb_dim = word_to_vec_map["miss"].shape[0]
print(emb_dim)
    ...
    for t in range(num_iterations):                       # Loop over the number of iterations
        for i in range(m):                                # Loop over the training examples
            ...
            for k in range(0, n_y):
                #ost = - Y_oh[k][i] @ np.log(a[k][i])
                cost = -np.sum(Y_oh[k] * np.log(a[k])) 
...
X_my_sentences = np.array(["i adore you", "i love cake", "funny lol", "lets play with a ball", "food is ready", "not feeling happy"])
